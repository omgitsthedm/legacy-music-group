 ...
 * Canonical source: agency-ops/templates/lifi-credit/LifiCredit.tsx
 * Self-contained: markup + styles, no deps, no runtime JS. Inherits colour via
 * currentColor so it reverses on any ground. All motion stops under
 * prefers-reduced-motion.
 */
import type { CSSProperties } from "react";

const LFC_CSS = String.raw`/* ==========================================================================
   Little Fight NYC — credit mark (animated tugboat)
...
export default function LifiCredit({
  ...
  return (
    <span
      className={className}
      style={size ? ({ "--lfc-size": size } as CSSProperties) : undefined}
    >
      <style dangerouslySetInnerHTML={{ __html: LFC_CSS }} />
      <span dangerouslySetInnerHTML={{ __html: LFC_MARKUP }} />
    </span>
  );
}
